...
# Creates table with specified name and column(s) Columns and their types are
# passed as keyword args as such: rank = "INTEGER", name = "TEXT"
def createTable(cursor, tableName, **kwargs):
    print(f"Creating {tableName} table...")
    createTableString = f"CREATE TABLE {tableName} ("
    # Every table will be created with a SQL ID column for an autoincrementing
    # counter
    ...
    while i < (len(kwargs) - 1):
        createTableString += f"{list(kwargs)[i]} "
        createTableString += f"{list(kwargs.values())[i]}, "
        i += 1
    createTableString += f"{list(kwargs)[i]} "
    createTableString += f"{list(kwargs.values())[i]})"
...
def insertRow(cursor, tableName, **kwargs):
    ...
    valuesArgs = "(NULL, "
    # Creates strings from the keyword arguments that SQLite can use as named
    # parameters
    i = 0
    while i < (len(kwargs) - 1):
        insertRowString += f"{list(kwargs)[i]}, "
        valuesArgs += f":{list(kwargs)[i]}, "
        i += 1
    insertRowString += f"{list(kwargs)[i]})"
    valuesArgs += f":{list(kwargs)[i]})"
    insertRowString += f" VALUES {valuesArgs}"
